...
export default function Inxdex(props) {
  ...
  let [accCookie, setAccCookie] = useState(
    urlParse != null && urlParse.account && urlParse.account[0]
      ? urlParse.account[0]
      : ''
  )
  ...
  function handleAccCookie() {
    ...
    var isMobile = false
    if (/^\d+$/.test(accCookie) && Validate.validateMobile(accCookie)) {
      // check string only numbers
      isMobile = true
    }
  ...
  function showPopUp() {
    ...
    let options = Validate.validateEmail(tmp_acc) ? (
      <div className='modal-body-new-phone'>
        <h3>
          Email <span className='blue-txt'>{tmp_acc}</span> chưa được đăng ký
        </h3>
        <p>
      ...
      <div className='modal-body-new-phone'>
        <h3>
          Số điện thoại <span className='blue-txt'>{tmp_acc}</span> chưa được
          đăng ký
        </h3>
        <p>
  ...
  function handleQuickLogin() {
    if (!account) {
      ...
      Validate.sendError(
        ...
        '',
        window.location.href,
        Constant.API_BASE_URL + 'quickLogin',
        '',
        '',
        'Vui lòng nhập Số điện thoại/Email.'
      )
  ...
  function getSecureQuickLogin(cb) {
    ...
    Validate.requestPost(
      ...
      function (error, dataRes) {
        if (window.ee)
          window.ee.removeListener('secureSDK', getSecureQuickLogin)
