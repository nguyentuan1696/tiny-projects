...
export default function AddMobile({ location }) {
  let dispatch = useDispatch()

  let accProps = location.state && location.state.acc ? location.state.acc : ''
  let mobileProps =
  ...
  //chay hieu ung scrollToTop
    // khi nao quay ve trang nay thi tu dong scroll len dau
  useEffect(() => {
  ...
  function goToIndex() {
    ...
    // luu thong tin user trong store
          ...
          <div className='modal-content'>
            <div className='new-phone-modal'>
              ...
              <div className='modal-body-new-phone'>
                <h3>
                  Số điện thoại <span className='blue-txt'>{mobile}</span> đã là
                  tài khoản ViệtID nên không thể cập nhật vào tài khoản này
                </h3>
                <p>
...
// gui sdk sendActionToNative voi param la so dien thoai
