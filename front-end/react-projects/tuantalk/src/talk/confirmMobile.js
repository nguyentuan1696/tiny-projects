...
export default function ConfirmMobile(props) {
    ...
    let hasMobile = (props.location.state && props.location.state.hasMobile) ? props.location.state.hasMobile : false;
    let isVietID = (props.location.state && props.location.state.isVietID) ? props.location.state.isVietID : false;

    let tmp_mobile = (fromProps === "index" || fromProps === "register") ? accProps : '';
    ...
    function getCallBack() {
        ...
        Validate.sendSuccess("click-getCallBack", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "getCallBack", JSON.stringify(dataKibana), "success");
        ...
        Validate.requestPost(Constant.API_BASE_URL + "getCallBack", data, function (error, dataRes) {
            if (error) {
                ...
                Validate.sendError("successLogin", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "getCallBack", JSON.stringify(data), "Đường truyền mạng không ổn định. Vui lòng thử lại.", JSON.stringify(data));
            } else {
                Validate.sendSuccess("successLogin", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "getCallBack", JSON.stringify(data), JSON.stringify(dataRes));
                if (dataRes.signal !== 0) {
    ...
    function loginByOTP2(otp) {
        if (tmp_mobile && fromProps !== "register" && fromProps !== "index") {
            ...
            if (!otp) {
                ...
                Validate.sendError("confirmMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmAccount", "", "Vui lòng nhập mã xác thực.");
                return;
            }
            ...
            Validate.sendSuccess("confirmMobile", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "confirmAccount", JSON.stringify(dataKibana), "success");

            Validate.requestPost(Constant.API_BASE_URL + "confirmAccount", data, function (error, dataRes) {
                if (error) {
                    setError("Đường truyền mạng không ổn định. Vui lòng thử lại.");
                    Validate.sendError("confirmMobile", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "confirmAccount", JSON.stringify(dataKibana), "Đường truyền mạng không ổn định. Vui lòng thử lại.", JSON.stringify(dataKibana));
                } else {
                    if (dataRes.signal !== 0) {
                        Validate.sendSuccess("confirmMobile", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "confirmAccount", JSON.stringify(dataKibana), JSON.stringify(dataRes));
                        ...
                        Validate.sendError("confirmMobile", mobileProps ? mobileProps : accProps, window.location.href, Constant.API_BASE_URL + "confirmAccount", JSON.stringify(dataKibana), JSON.stringify(dataRes));
            ...
            if (!otp) {
                setError("Vui lòng nhập mã xác thực.");
                Validate.sendError("confirmCheckMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmCheckMobile", "", "Vui lòng nhập mã xác thực.");
                return;
            }
            ...
            Validate.sendSuccess("confirmCheckMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmCheckMobile", JSON.stringify(dataKibana2), "success");

            Validate.requestPost(Constant.API_BASE_URL + "confirmCheckMobile", data1, function (error, dataRes) {
                if (error) {
                    setError("Đường truyền mạng không ổn định. Vui lòng thử lại.");
                    Validate.sendError("confirmCheckMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmCheckMobile", JSON.stringify(dataKibana2), "Đường truyền mạng không ổn định. Vui lòng thử lại.", JSON.stringify(dataKibana));
                } else {
                    if (dataRes.signal !== 0) {
                        Validate.sendSuccess("confirmCheckMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmCheckMobile", JSON.stringify(dataKibana2), JSON.stringify(dataRes));

                        if (fromProps === "addMobile" || fromProps === "updateMobileApple") {
                            setPopUp(true);
                        }
                        ...
                        Validate.sendError("confirmCheckMobile", accProps, window.location.href, Constant.API_BASE_URL + "confirmCheckMobile", JSON.stringify(dataKibana2), JSON.stringify(dataRes));
                        setError(dataRes.message);
                        if (dataRes.message === "Quá số lần nhập sai. Vui lòng thử lại sau 5 phút.") {
                            restart();
                        }
    ...
    function handleResendOTP() {
        ...
        if (tmp_time === 0 || tmp_time < 0) {
            return (<div className="resent" onClick={() => resendOTP(mobileProps ? mobileProps : accProps)}>Gửi lại mã OTP</div>);
        } else {
    ...
    function getSecure(cb) {
        ...
        Validate.requestPost(Constant.API_BASE_URL + "resendOTP", data, function (error, dataRes) {
            ...
            if (error) {
                setError("Đường truyền mạng không ổn định. Vui lòng thử lại.");
                Validate.sendError("confirmMobile", accSecure ? accSecure : window.account, window.location.href, Constant.API_BASE_URL + "resendOTP", JSON.stringify(data), "Đường truyền mạng không ổn định. Vui lòng thử lại.", JSON.stringify(data));
            } else {
                if (dataRes.signal !== 0) {
                    Validate.sendSuccess("confirmMobile", accSecure ? accSecure : window.account, window.location.href, Constant.API_BASE_URL + "resendOTP", JSON.stringify(data), JSON.stringify(dataRes));
                } else {
                    setError("Quá số lần gửi.");
                    Validate.sendError("confirmMobile", accSecure ? accSecure : window.account, window.location.href, Constant.API_BASE_URL + "resendOTP", JSON.stringify(data), JSON.stringify(dataRes));
                }
            }
        });
    }
    ...
    if (isLogin.signal === 1) {
        ...
        switch (fromProps) {
            case "addMobile":
                back = (<Link className="img-back" to={{ pathname: '/talk/addMobile', state: dataState }} onClick={() => hideKeyBoard()}><img src="https://mingid.mediacdn.vn/king/image/back.png" alt="" /></Link>);
                break;
            case "updateMobile":
                back = (<Link className="img-back" to={{ pathname: '/talk/updateMobile', state: dataState }} onClick={() => hideKeyBoard()}><img src="https://mingid.mediacdn.vn/king/image/back.png" alt="" /></Link>);
                break;
            case "updateMobileApple":
                back = (<Link className="img-back" to={{ pathname: '/talk/updateMobileApple', state: dataState }} onClick={() => hideKeyBoard()}><img src="https://mingid.mediacdn.vn/king/image/back.png" alt="" /></Link>);
                break;
            case "register":
                back = (<Link className="img-back" to="/talk/registerAccount" onClick={() => hideKeyBoard()}><img src="https://mingid.mediacdn.vn/king/image/back.png" alt="" /></Link>);
                break;
            default:
                back = (<Link className="img-back" to={{ pathname: '/talk/index' }} onClick={() => hideKeyBoard()}><img src="https://mingid.mediacdn.vn/king/image/back.png" alt="" /></Link>);
                break;
        }

        let classImg = (fromProps === "register" || isVietID) ?
        ...
        return (
            <div className="wrapper-enter-otp">
                ...
                <div className={classTitle}>Nhập mã OTP được gửi về Số điện thoại <span>{tmp_mobile ? tmp_mobile : mobileProps}</span></div>
                ...
                <div className={className}>
                    <div className="otp-ok-modal">
                        <div className="modal-body">
                            <p>Số điện thoại {tmp_mobile ? tmp_mobile : mobileProps} đã được cập nhật thành công. Bạn sẽ sử dụng để đăng nhập lần tới</p>
                        </div>
                        <div className="modal-footer">
                            <p className="btn-add-mobile" onClick={() => handleAddMobileSuccess()}>Đồng ý</p>
                        </div>
                    </div>
                </div>
            </div>
